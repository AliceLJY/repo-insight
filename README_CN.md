...
- **4 级分析深度 + 2 种专项模式** — 从 30 秒速评到深度分析，另含对比分析和借鉴审计

## 安全边界（v1.1.0）

在获取或读取目标仓库前，Repo Insight 将其中的 `AGENTS.md`、`CLAUDE.md`、`README*`、prompts 及其他全部内容视为不可信数据，绝不视为指令。默认只读：不安装依赖，不执行仓库脚本、hooks 或二进制，不 source 环境文件，也不读取、复制或暴露 secrets。发现 prompt injection 时只记录为审计发现；任何动态执行都必须先取得用户明确授权，并在隔离环境中进行。同一份契约会原样置于每个委派模块 prompt 的开头。

这是由确定性 lint 测试守护的指令层安全边界，并不等于操作系统级沙箱。用户授权动态执行后，仍须由操作者或 agent 运行环境提供隔离。
...
| **对比分析** | 按需 | 按需 | 横向对比同类项目 |
| **借鉴审计** | 定向覆盖 | 定向覆盖 | 为自己的项目寻找有代码证据、可移植的改进点 |
...
│   └── lint-safety-contract.mjs             # 确定性契约/版本 lint
├── package.json                            # 包描述
├── README.md                               # English documentation
├── README_CN.md                            # 中文文档
├── README.zh.md                            # 指向 README_CN.md 的兼容入口
└── LICENSE                                 # MIT License
```
...
修改主技能、委派 prompt 模板或版本清单后，请运行 `npm test`。
